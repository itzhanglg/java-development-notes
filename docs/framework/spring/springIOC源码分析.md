...
- 断点（观察调用栈）
- 反调(（Find Usages）
- 经验（Spring框架中doXXX方法，真真做处理的地方）

> 原文链接：[Spring IOC 容器源码分析](https://javadoop.com/post/spring-ioc)

本文要说的 IOC 总体来说有两处地方最重要，一个是创建 Bean 容器，一个是初始化 Bean，如果读者觉得一次性看完本文压力有点大，那么可以按这个思路分两次消化。读者不一定对 Spring 容器的源码感兴趣，也许附录部分介绍的知识对读者有些许作用。
...
![img](https://gitee.com/itzlg/mypictures/raw/master/img/1589016154641-a312a536-01af-43a0-b5ad-2abb33344bb2.png)
...
![img](https://gitee.com/itzlg/mypictures/raw/master/img/1589016154079-03353372-598b-4731-84c2-7d07357966ef.png)
...
1. ApplicationContext 继承了 HierarchicalBeanFactory，Hierarchical 单词本身已经能说明问题了，也就是说我们可以在应用中起多个 BeanFactory，然后可以将各个 BeanFactory 设置为父子关系。
...
1. ConfigurableListableBeanFactory 也是一个特殊的接口，看图，特殊之处在于它继承了第二层所有的三个接口，而 ApplicationContext 没有。这点之后会用到。
...
![image-20201006113248654](https://gitee.com/itzlg/mypictures/raw/master/img/image-20201006113248654.png)
...
### 4.准备 Bean 容器: prepareBeanFactory
...
> 上面的返回值用了 MyCommand，当然，如果 Command 只有一个实现类，那返回值也可以写Command。
